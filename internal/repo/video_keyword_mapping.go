...
func NewVideoKeywordMappingRepo(data *Data) service.VideoKeywordMappingRepo {
	return &videoKeywordMappingRepo{
		data: data,
	}
}

func (ro videoKeywordMappingRepo) Create(ctx context.Context, item *service.CreateVideoKeywordMapping) (err error) {
...
func (ro videoKeywordMappingRepo) Get(ctx context.Context, id uint64) (item *service.VideoKeywordMapping, err error) {
	item = &service.VideoKeywordMapping{}
	p := query.Use(ro.data.DB(ctx)).VideoKeywordMapping
	db := p.WithContext(ctx)
	m := db.GetByID(id)
...
func (ro videoKeywordMappingRepo) Find(ctx context.Context, condition *service.FindVideoKeywordMapping) (rp []service.VideoKeywordMapping) {
	p := query.Use(ro.data.DB(ctx)).VideoKeywordMapping
	db := p.WithContext(ctx)
	rp = make([]service.VideoKeywordMapping, 0)
...
func (ro videoKeywordMappingRepo) Update(ctx context.Context, item *service.UpdateVideoKeywordMapping) (err error) {
	p := query.Use(ro.data.DB(ctx)).VideoKeywordMapping
	db := p.WithContext(ctx)
