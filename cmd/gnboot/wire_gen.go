...
func wireApp(c *conf.Bootstrap) (*kratos.App, func(), error) {
	...
	episodeService := service.NewEpisodeService(c, videoRepo, actorRepo, videoActorMappingRepo, episodeSubtitleMappingRepo, episodeRepo)
	...
	grpcServer := server.NewGRPCServer(c, videoProvider, episodeProvider, userProvider)
	httpServer := server.NewHTTPServer(c, videoProvider, episodeProvider, userProvider)
