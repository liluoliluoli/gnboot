...
func NewStudioRepo(data *Data) biz.StudioRepo {
	return &studioRepo{
		data: data,
	}
}

func (ro studioRepo) Create(ctx context.Context, item *biz.CreateStudio) (err error) {
...
func (ro studioRepo) Get(ctx context.Context, id uint64) (item *biz.Studio, err error) {
	item = &biz.Studio{}
	p := query.Use(ro.data.DB(ctx)).Studio
	db := p.WithContext(ctx)
	m := db.GetByID(id)
...
func (ro studioRepo) Find(ctx context.Context, condition *biz.FindStudio) (rp []biz.Studio) {
	p := query.Use(ro.data.DB(ctx)).Studio
	db := p.WithContext(ctx)
	rp = make([]biz.Studio, 0)
...
func (ro studioRepo) Update(ctx context.Context, item *biz.UpdateStudio) (err error) {
	p := query.Use(ro.data.DB(ctx)).Studio
	db := p.WithContext(ctx)
