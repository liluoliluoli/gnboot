...
func NewKeywordRepo(data *Data) service.KeywordRepo {
	return &keywordRepo{
		data: data,
	}
}

func (ro keywordRepo) Create(ctx context.Context, item *service.CreateKeyword) (err error) {
...
func (ro keywordRepo) Get(ctx context.Context, id uint64) (item *service.Keyword, err error) {
...
func (ro keywordRepo) Find(ctx context.Context, condition *service.FindKeyword) (rp []service.Keyword) {
	p := query.Use(ro.data.DB(ctx)).Keyword
	db := p.WithContext(ctx)
	rp = make([]service.Keyword, 0)
...
func (ro keywordRepo) Update(ctx context.Context, item *service.UpdateKeyword) (err error) {
	p := query.Use(ro.data.DB(ctx)).Keyword
	db := p.WithContext(ctx)
