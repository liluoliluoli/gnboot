...
func NewVideoStudioMappingRepo(data *Data) biz.VideoStudioMappingRepo {
	return &videoStudioMappingRepo{
		data: data,
	}
}

func (ro videoStudioMappingRepo) Create(ctx context.Context, item *biz.CreateVideoStudioMapping) (err error) {
...
func (ro videoStudioMappingRepo) Get(ctx context.Context, id uint64) (item *biz.VideoStudioMapping, err error) {
	item = &biz.VideoStudioMapping{}
	p := query.Use(ro.data.DB(ctx)).VideoStudioMapping
	db := p.WithContext(ctx)
	m := db.GetByID(id)
...
func (ro videoStudioMappingRepo) Find(ctx context.Context, condition *biz.FindVideoStudioMapping) (rp []biz.VideoStudioMapping) {
	p := query.Use(ro.data.DB(ctx)).VideoStudioMapping
	db := p.WithContext(ctx)
	rp = make([]biz.VideoStudioMapping, 0)
...
func (ro videoStudioMappingRepo) Update(ctx context.Context, item *biz.UpdateVideoStudioMapping) (err error) {
	p := query.Use(ro.data.DB(ctx)).VideoStudioMapping
	db := p.WithContext(ctx)
